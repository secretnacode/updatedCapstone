...
> => {
  try {
    return (
      await pool.query(
        `select o."orgId", o."orgName", concat(f."farmerFirstName", ' ', f."farmerLastName") as "farmerLeaderName", f."farmerAlias" as "farmerLeaderAlias", f."farmerId", count(m."orgId") as "totalMember" from capstone.org o join capstone.farmer f on o."farmerLeadId" = f."farmerId" join capstone.farmer m on o."orgId" = m."orgId" group by o."orgId", o."orgName", f."farmerFirstName", f."farmerLastName", f."farmerId"`
      )
    ).rows;
  } catch (error) {
    console.error(
